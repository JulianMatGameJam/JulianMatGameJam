...
public class Player : KinematicBody2D
{
	...
	{
		return GetNode<Sprite>("Screamer");
	}
	...
	public override void _Ready()
	{
		...
		stepsound = GetParent().GetNode<AudioStreamPlayer>("Music/StepSound");
	}
	...
	private void _on_Area2D2_body_entered(object body)
	{
		if (body == this)
		{
			...
			var timer = GetParent().GetNode<Timer>("Timer");
			timer.Start();
			son = GetNode<AudioStreamPlayer>("SonScreamer");
			son.Play();
			
			Spider spider = (Spider) GetTree().GetNodesInGroup("Spider")[0];
			spider.Position = new Vector2(-16, 369);
		}
	}
	...
	private bool condaudio = false;
	
	private void Stepsound()
	{
		if (condaudio)
		{
			if (IsOnFloor() == true && (Input.IsActionPressed("ui_right") || Input.IsActionPressed("ui_left")))
			{
				stepsound.Play();
			}
			else
	...
	private void _on_SoundTimer_timeout()
	{
		condaudio = true;
	}
	...
	private void EndGame()
	{
		if (Position.x >= 14635)
		{
			gameisfinished = true;
			anim_mod = "Idle";
			EmitSignal("GameIsFinish",true);
		}
	}

	[Signal]

	private delegate void GameIsFinish(bool cond);
	...
	public override void _Process(float delta)
	{
		...
		if (gamehasstarted && gameisfinished == false )
		{
			Move(delta);
		}
		...
		*/
		if (!alive)
		{
			...
			Position = lastpos;
			Spider spider = (Spider) GetTree().GetNodesInGroup("Spider")[0];
			dead = GetNode<AudioStreamPlayer>("Tadeadçachacal");
			dead.Play();

			spider.Res(Position.x, spider.Position.y);
		}
	}
}
